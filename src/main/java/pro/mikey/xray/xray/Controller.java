...
public class Controller {
    ...
    private static Vec3 lastXyzPos = null;
    ...
    public static int getHalfRange() {
        return (int) Math.max(0, getRadius()/2);
    }
    ...
    private static boolean playerHasMoved() {
        ...
        Vec3 plyBlockPos = Minecraft.getInstance().player.position();
        int chunkRange = (int) (getHalfRange() * Configuration.general.updateRadiusScale.get());



        return lastChunkPos == null ||
                plyChunkPos.x > lastChunkPos.x + chunkRange || plyChunkPos.x < lastChunkPos.x - chunkRange ||
                plyChunkPos.z > lastChunkPos.z + chunkRange || plyChunkPos.z < lastChunkPos.z - chunkRange;
    }

    private static void updatePlayerPosition() {
        lastChunkPos = Minecraft.getInstance().player.chunkPosition();
        lastXyzPos = Minecraft.getInstance().player.position();
    }
